...
void BaseApplication::framebufferResizeCallback(GLFWwindow* window, int width, int height) {
...
void BaseApplication::populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
    createInfo = {};
...
VKAPI_ATTR VkBool32 VKAPI_CALL BaseApplication::debugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageType, const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData, void* pUserData) {
...
VkSampleCountFlagBits BaseApplication::getMaxUsableSampleCount() {
...
QueueFamilyIndices BaseApplication::findQueueFamilies(VkPhysicalDevice device) const {
...
VkExtent2D BaseApplication::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities) const {
...
void BaseApplication::createSwapChain() {
    ...
    MAX_FRAMES_IN_FLIGHT = static_cast<int>(std::max(imageCount, 2u));
...
SwapChainSupportDetails BaseApplication::querySwapChainSupport(VkPhysicalDevice device) const {
...
VkImageView BaseApplication::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) const {
...
void BaseApplication::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels) {
...
void BaseApplication::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
...
void BaseApplication::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
VkCommandBuffer BaseApplication::beginSingleTimeCommands() const {
    VkCommandBufferAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
    allocInfo.commandPool = commandPool;
...
void BaseApplication::endSingleTimeCommands(VkCommandBuffer commandBuffer, bool useComputeQueue) const {
    vkEndCommandBuffer(commandBuffer);

    VkQueue commandQueue = useComputeQueue ? computeQueue : graphicsQueue;
...
uint32_t BaseApplication::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) const {
...
VkShaderModule BaseApplication::createShaderModule(const std::vector<char>& code) const {
...
VkFormat BaseApplication::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) const {
...
VkFormat BaseApplication::findDepthFormat() const {
...
bool BaseApplication::hasStencilComponent(VkFormat format) {
    return format == VK_FORMAT_D32_SFLOAT_S8_UINT || format == VK_FORMAT_D24_UNORM_S8_UINT;
}

void BaseApplication::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) const {
    VkBufferCreateInfo bufferInfo{};
    bufferInfo.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
