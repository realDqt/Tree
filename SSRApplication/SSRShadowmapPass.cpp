...
void SSRShadowmapPass::createFramebuffers() {
    ...
    for (size_t i = 0; i < swapChainImagesCount; i++) {
        ...
        framebufferInfo.width = SM_RESOLUTION;
        framebufferInfo.height = SM_RESOLUTION;
...
void SSRShadowmapPass::createGraphicsPipeline(){
    auto vertShaderCode = readFile("E:/clion_proj/Tree/shaders/SSR/shadowVert.spv");
    auto fragShaderCode = readFile("E:/clion_proj/Tree/shaders/SSR/shadowFrag.spv");
...
void SSRShadowmapPass::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    renderPassInfo.renderArea.extent = VkExtent2D(SM_RESOLUTION, SM_RESOLUTION);
    ...
    viewport.x = 0.0f;
    viewport.y = 0.0f;
    viewport.width = (float) SM_RESOLUTION;
    viewport.height = (float) SM_RESOLUTION;
...
void SSRShadowmapPass::updateUniformBuffer(uint32_t currentImage){
    ...
    proj[1][1] *= -1;
    ubo.lightVP = proj * view;

    ubo.cubeModel = cubeModel;
    ubo.floorModel = floorModel;
