...
private:
    ...
    std::vector<VkFramebuffer> SMframebuffers;
    std::vector<VkFramebuffer> LVswapChainFramebuffers;


    // shadow map
    VkRenderPass SMrenderPass;
    VkDescriptorSetLayout SMdescriptorSetLayout;
    VkPipelineLayout SMpipelineLayout;
    VkPipeline SMgraphicsPipeline;
    ...
    // LV
    VkRenderPass LVrenderPass;
    VkDescriptorSetLayout LVdescriptorSetLayout;
    VkPipelineLayout LVpipelineLayout;
    VkPipeline LVgraphicsPipeline;
    ...
    // shadow map
    std::vector<VkBuffer> SMuniformBuffers;
    std::vector<VkDeviceMemory> SMuniformBuffersMemory;
    std::vector<void*> SMuniformBuffersMapped;

    // LV
    std::vector<VkBuffer> LVuniformBuffers;
    std::vector<VkDeviceMemory> LVuniformBuffersMemory;
    std::vector<void*> LVuniformBuffersMapped;

    // shadow map
    VkDescriptorPool SMdescriptorPool;
    std::vector<VkDescriptorSet> SMdescriptorSets;
    ...
    // LV
    VkDescriptorPool LVdescriptorPool;
    std::vector<VkDescriptorSet> LVdescriptorSets;

    std::vector<VkCommandBuffer> commandBuffers;

    std::vector<VkSemaphore> smAvailableSemaphores;
    ...
    static void framebufferResizeCallback(GLFWwindow* window, int width, int height) {
        auto app = reinterpret_cast<PCSSApplication*>(glfwGetWindowUserPointer(window));
    ...
    void initVulkan() {
        ...
        createSMDescriptorSetLayout();
        createLVDescriptorSetLayout();

        createGraphicsPipeline();
        createSMGraphicsPipeline();
        createLVGraphicsPipeline();
        ...
        createFramebuffers();
        createSMFramebuffers();
        createLVFramebuffers();
        createTextureImage();
        ...
        createSMUniformBuffers();
        createLVUniformBuffers();

        createLightBuffers();
        createDescriptorPool();
        createSMDescriptorPool();
        createLVDescriptorPool();
        createDescriptorSets();
        createSMDescriptorSets();
        createLVDescriptorSets();
    ...
    void cleanupSwapChain() {
        ...
        for (auto framebuffer : SMframebuffers) {
            vkDestroyFramebuffer(device, framebuffer, nullptr);
        }

        for (auto framebuffer : LVswapChainFramebuffers) {
            vkDestroyFramebuffer(device, framebuffer, nullptr);
        }
    ...
    void cleanup() {
        ...
        vkDestroyPipeline(device, SMgraphicsPipeline, nullptr);
        vkDestroyPipelineLayout(device, SMpipelineLayout, nullptr);
        vkDestroyRenderPass(device, SMrenderPass, nullptr);

        vkDestroyPipeline(device, LVgraphicsPipeline, nullptr);
        vkDestroyPipelineLayout(device, LVpipelineLayout, nullptr);
        vkDestroyRenderPass(device, LVrenderPass, nullptr);
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            vkDestroyBuffer(device, SMuniformBuffers[i], nullptr);
            vkFreeMemory(device, SMuniformBuffersMemory[i], nullptr);

            vkDestroyBuffer(device, LVuniformBuffers[i], nullptr);
            vkFreeMemory(device, LVuniformBuffersMemory[i], nullptr);
        }
        ...
        vkDestroyDescriptorPool(device, LVdescriptorPool, nullptr);

        vkDestroyDescriptorSetLayout(device, descriptorSetLayout, nullptr);
        vkDestroyDescriptorSetLayout(device, SMdescriptorSetLayout, nullptr);
        vkDestroyDescriptorSetLayout(device, LVdescriptorSetLayout, nullptr);
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            vkDestroySemaphore(device, smAvailableSemaphores[i], nullptr);
    ...
    void createSMRenderPass(){
        ...
        if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &SMrenderPass) != VK_SUCCESS) {
            throw std::runtime_error("failed to create render pass!");
        }
    }


    void createLVRenderPass() {
        VkAttachmentDescription colorAttachment{};
        colorAttachment.format = swapChainImageFormat;
        colorAttachment.samples = msaaSamples;
        ...
        if (vkCreateRenderPass(device, &renderPassInfo, nullptr, &LVrenderPass) != VK_SUCCESS) {
            throw std::runtime_error("failed to create render pass!");
        }
    }

    void createLVDescriptorSetLayout()
    {
        ...
        if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &LVdescriptorSetLayout) != VK_SUCCESS) {
    ...
    void createDescriptorSetLayout() {
        ...
        VkDescriptorSetLayoutBinding SMsamplerLayoutBinding{};
        SMsamplerLayoutBinding.binding = 3;
        SMsamplerLayoutBinding.descriptorCount = 1;
        SMsamplerLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
        SMsamplerLayoutBinding.pImmutableSamplers = nullptr;
        SMsamplerLayoutBinding.stageFlags = VK_SHADER_STAGE_FRAGMENT_BIT;

        std::array<VkDescriptorSetLayoutBinding, 4> bindings = {uboLayoutBinding, samplerLayoutBinding, lightLayoutBinding, SMsamplerLayoutBinding};
        VkDescriptorSetLayoutCreateInfo layoutInfo{};
        layoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
        layoutInfo.bindingCount = static_cast<uint32_t>(bindings.size());
        layoutInfo.pBindings = bindings.data();
    ...
    void createSMDescriptorSetLayout()
    {
        VkDescriptorSetLayoutBinding uboLayoutBinding{};
        uboLayoutBinding.binding = 0;
        uboLayoutBinding.descriptorCount = 1;
        ...
        if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &SMdescriptorSetLayout) != VK_SUCCESS) {
            throw std::runtime_error("failed to create descriptor set layout!");
        }
    }

    void createLVGraphicsPipeline(){
        ...
        auto bindingDescription = VertexLight::getBindingDescription();
        auto attributeDescriptions = VertexLight::getAttributeDescriptions();
        ...
        pipelineLayoutInfo.pSetLayouts = &LVdescriptorSetLayout;


        if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &LVpipelineLayout) != VK_SUCCESS) {
        ...
        pipelineInfo.layout = LVpipelineLayout;
        pipelineInfo.renderPass = LVrenderPass;
        pipelineInfo.subpass = 0;
        pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;

        if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &LVgraphicsPipeline) != VK_SUCCESS) {
    ...
    void createSMGraphicsPipeline(){
        auto vertShaderCode = readFile("E:/clion_proj/Tree/shaders/Marry/shadowVert.spv");
        auto fragShaderCode = readFile("E:/clion_proj/Tree/shaders/Marry/shadowFrag.spv");
        ...
        pipelineLayoutInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_LAYOUT_CREATE_INFO;
        pipelineLayoutInfo.setLayoutCount = 1;
        pipelineLayoutInfo.pSetLayouts = &SMdescriptorSetLayout;


        if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &SMpipelineLayout) != VK_SUCCESS) {
            throw std::runtime_error("failed to create pipeline layout!");
        }
        ...
        pipelineInfo.pColorBlendState = &colorBlending;
        pipelineInfo.pDynamicState = &dynamicState;
        pipelineInfo.layout = SMpipelineLayout;
        pipelineInfo.renderPass = SMrenderPass;
        pipelineInfo.subpass = 0;
        pipelineInfo.basePipelineHandle = VK_NULL_HANDLE;

        if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &SMgraphicsPipeline) != VK_SUCCESS) {
            throw std::runtime_error("failed to create graphics pipeline!");
        }
    ...
    void createSMFramebuffers() {
        SMframebuffers.resize(swapChainImageViews.size());
    ...
    void createLVFramebuffers() {
        LVswapChainFramebuffers.resize(swapChainImageViews.size());
        ...
        for (size_t i = 0; i < swapChainImageViews.size(); i++) {
            ...
            VkFramebufferCreateInfo framebufferInfo{};
            framebufferInfo.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO;
            framebufferInfo.renderPass = LVrenderPass;
            framebufferInfo.attachmentCount = static_cast<uint32_t>(attachments.size());
            framebufferInfo.pAttachments = attachments.data();
            framebufferInfo.width = swapChainExtent.width;
            framebufferInfo.height = swapChainExtent.height;
            ...
            if (vkCreateFramebuffer(device, &framebufferInfo, nullptr, &LVswapChainFramebuffers[i]) != VK_SUCCESS) {
                throw std::runtime_error("failed to create framebuffer!");
            }
        }
    }
    ...
    void createSMUniformBuffers(){
        VkDeviceSize bufferSize = sizeof(SMUniformBufferObject);

        SMuniformBuffers.resize(MAX_FRAMES_IN_FLIGHT);
        SMuniformBuffersMemory.resize(MAX_FRAMES_IN_FLIGHT);
        SMuniformBuffersMapped.resize(MAX_FRAMES_IN_FLIGHT);

        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, SMuniformBuffers[i], SMuniformBuffersMemory[i]);

            vkMapMemory(device, SMuniformBuffersMemory[i], 0, bufferSize, 0, &SMuniformBuffersMapped[i]);
        }
    }

    void createLVUniformBuffers(){
        VkDeviceSize bufferSize = sizeof(LVUniformBufferObject);

        LVuniformBuffers.resize(MAX_FRAMES_IN_FLIGHT);
        LVuniformBuffersMemory.resize(MAX_FRAMES_IN_FLIGHT);
        LVuniformBuffersMapped.resize(MAX_FRAMES_IN_FLIGHT);

        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, LVuniformBuffers[i], LVuniformBuffersMemory[i]);

            vkMapMemory(device, LVuniformBuffersMemory[i], 0, bufferSize, 0, &LVuniformBuffersMapped[i]);
        }
    }

    void createLightBuffers(){
        VkDeviceSize bufferSize = sizeof(DirectionalLight);
    ...
    void createSMDescriptorPool() {
        ...
        if (vkCreateDescriptorPool(device, &poolInfo, nullptr, &SMdescriptorPool) != VK_SUCCESS) {
            throw std::runtime_error("failed to create descriptor pool!");
        }
    }

    void createLVDescriptorPool() {
        std::array<VkDescriptorPoolSize, 1> poolSizes{};
        poolSizes[0].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        poolSizes[0].descriptorCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT);
        ...
        if (vkCreateDescriptorPool(device, &poolInfo, nullptr, &LVdescriptorPool) != VK_SUCCESS) {
            throw std::runtime_error("failed to create descriptor pool!");
        }
    }
    ...
    void createDescriptorSets() {
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            descriptorWrites[3].pImageInfo = &SMimageInfo;

            vkUpdateDescriptorSets(device, static_cast<uint32_t>(descriptorWrites.size()), descriptorWrites.data(), 0, nullptr);
        }
    }

    void createSMDescriptorSets() {
        std::vector<VkDescriptorSetLayout> layouts(MAX_FRAMES_IN_FLIGHT, SMdescriptorSetLayout);
        VkDescriptorSetAllocateInfo allocInfo{};
        allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
        allocInfo.descriptorPool = SMdescriptorPool;
        allocInfo.descriptorSetCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT);
        allocInfo.pSetLayouts = layouts.data();

        SMdescriptorSets.resize(MAX_FRAMES_IN_FLIGHT);
        if (vkAllocateDescriptorSets(device, &allocInfo, SMdescriptorSets.data()) != VK_SUCCESS) {
            throw std::runtime_error("failed to allocate descriptor sets!");
        }

        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            VkDescriptorBufferInfo bufferInfo{};
            bufferInfo.buffer = SMuniformBuffers[i];
            bufferInfo.offset = 0;
            ...
            descriptorWrites[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            descriptorWrites[0].dstSet = SMdescriptorSets[i];
            descriptorWrites[0].dstBinding = 0;
            descriptorWrites[0].dstArrayElement = 0;
            descriptorWrites[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
            descriptorWrites[0].descriptorCount = 1;
    ...
    void createLVDescriptorSets() {
        std::vector<VkDescriptorSetLayout> layouts(MAX_FRAMES_IN_FLIGHT, LVdescriptorSetLayout);
        VkDescriptorSetAllocateInfo allocInfo{};
        allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
        allocInfo.descriptorPool = LVdescriptorPool;
        allocInfo.descriptorSetCount = static_cast<uint32_t>(MAX_FRAMES_IN_FLIGHT);
        allocInfo.pSetLayouts = layouts.data();

        LVdescriptorSets.resize(MAX_FRAMES_IN_FLIGHT);
        if (vkAllocateDescriptorSets(device, &allocInfo, LVdescriptorSets.data()) != VK_SUCCESS) {
            throw std::runtime_error("failed to allocate descriptor sets!");
        }

        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            VkDescriptorBufferInfo bufferInfo{};
            bufferInfo.buffer = LVuniformBuffers[i];
            bufferInfo.offset = 0;
            bufferInfo.range = sizeof(LVUniformBufferObject);

            std::array<VkWriteDescriptorSet, 1> descriptorWrites{};

            descriptorWrites[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            descriptorWrites[0].dstSet = LVdescriptorSets[i];
            descriptorWrites[0].dstBinding = 0;
            descriptorWrites[0].dstArrayElement = 0;
            descriptorWrites[0].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
            descriptorWrites[0].descriptorCount = 1;
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, SMgraphicsPipeline);
        ...
        vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, SMpipelineLayout, 0, 1, &SMdescriptorSets[currentFrame], 0, nullptr);
        ...
        renderPassInfo = VkRenderPassBeginInfo {};
        renderPassInfo.sType = VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO;
        renderPassInfo.renderPass = LVrenderPass;
        renderPassInfo.framebuffer = LVswapChainFramebuffers[imageIndex];
        renderPassInfo.renderArea.offset = {0, 0};
        renderPassInfo.renderArea.extent = swapChainExtent;
        ...
        vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, LVgraphicsPipeline);

        VkBuffer vertexBuffers3[] = {vertexBuffer3};
        VkDeviceSize offsets3[] = {0};
        updateLVUniformBuffer(currentFrame);
        vkCmdBindVertexBuffers(commandBuffer, 0, 1, vertexBuffers3, offsets3);

        vkCmdBindIndexBuffer(commandBuffer, indexBuffer3, 0, VK_INDEX_TYPE_UINT32);

        vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, LVpipelineLayout, 0, 1, &LVdescriptorSets[currentFrame], 0, nullptr);

        vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(indices3.size()), 1, 0, 0, 0);
    ...
    void createSyncObjects() {
        smAvailableSemaphores.resize(MAX_FRAMES_IN_FLIGHT);
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &imageAvailableSemaphores[i]) != VK_SUCCESS ||
                ...
                vkCreateSemaphore(device, &semaphoreInfo, nullptr, &smAvailableSemaphores[i]) != VK_SUCCESS ||
    ...
    void updateLVUniformBuffer(uint32_t currentImage){
        LVUniformBufferObject ubo{};
        ...
        ubo.lightIntensity = light.lightIntensity;
        memcpy(LVuniformBuffersMapped[currentImage], &ubo, sizeof(ubo));
    }
